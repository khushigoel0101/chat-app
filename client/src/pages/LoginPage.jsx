    ...
    <div className='min-h-screen bg-cover bg-center flex items-center justify-center gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl'>
      ...
      <form onSubmit={onSubmitHandler} className='border-2 bg-white/8 text-white border-gray-500 p-6 flex flex-col gap-6 rounded-lg shadow-lg'>
         <h2 className='font-medium text-2xl flex justify-between items-center'>
          {currState}
          {isDataSubmitted && <img onClick={()=> setIsDataSubmitted(flase)} src={assets.arrow_icon} alt="" className='w-5 cursor-pointer' /> }
          </h2>
          ...
          {!isDataSubmitted && (
            ...
            <input onChange={(e)=> setEmail(e.target.value)} value = {email}
             type = "password" placeholder='Password' required className='p-2 border boreder-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 '/>
          ...
          <div>
            <input type = "checkbox" />
            <p>Agree to the ters of use and privacy policy.</p>
          </div>
          ...
          <div className='flex flex-col gap-2'>
            {currState === "Sign up" ? (
              ...
              <p className='text-sm text-gay-600'>Create an account <span onClick={() => setCurrState("Sign up")}
